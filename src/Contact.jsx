...
const ACCESS_KEY = 'c05f3b42-aecc-4a0d-8dd0-d2ef45312618';
        ...
        <ContactGrid>
          ...
          <Form onSubmit={handleSubmit}>
            <AnimatedField delay="0.15s">
              <FormRow>
                <StyledTextField id="name" name="name" label="Name" variant="outlined" required />
                <StyledTextField id="email" name="email" label="Email" type="email" variant="outlined" required />
              </FormRow>
            </AnimatedField>
            <AnimatedField delay="0.30s">
              <StyledTextField id="subject" name="subject" label="Subject" variant="outlined" required />
            </AnimatedField>
            <AnimatedField delay="0.45s">
              <StyledTextField id="message" name="message" label="Message" variant="outlined" multiline rows={4} required />
            </AnimatedField>
            <FormSubmit type="submit" disabled={status === 'sending'}>
              {status === 'sending' ? 'Sending...' : (<>Send Message <SendIcon /></>)}
            </FormSubmit>
            {status === 'success' && (
              <SuccessMsg><CheckCircleOutlineIcon /> Message sent successfully!</SuccessMsg>
            )}
            {status === 'error' && (
              <SuccessMsg style={{ color: '#ef4444' }}>Oops! Something went wrong. Please try again.</SuccessMsg>
            )}
          </Form>
        </ContactGrid>
      </Inner>
